...
object SideOutWaterMarkDemo {
  def main(args: Array[String]): Unit = {
    ...
    //设置侧输出流标志
    val outputTag = new OutputTag[Car]("late")
    ...
    //获取侧道输出流的数据
    val sideOutDS = windowDS.getSideOutput(outputTag)
    sideOutDS.printToErr("迟到的数据：")
