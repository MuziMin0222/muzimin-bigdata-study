 ...
 *       演示自定义非并行数据源实现
 **/
object MySourceParalle {
  def main(args: Array[String]): Unit = {
    val senv = StreamExecutionEnvironment.getExecutionEnvironment

    //添加自定义的数据源
    //setParallelism如果设置的个数大于1  会出错，因为该数据源不是并行的
    val sourceDS = senv.addSource(new MyParalleSourceFunction).setParallelism(2)
    //打印数据
    sourceDS.print()
    senv.execute()
  }
...
class MyParalleSourceFunction extends ParallelSourceFunction[Int] {
  //发送数据，生产数据的方法
  var count: Int = 0
  var flag = true
